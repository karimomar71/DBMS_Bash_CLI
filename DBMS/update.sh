...
sql_line=$(echo "$entry" | sed -e 's/UPDATE//g' -e 's/SET//g' -e 's/WHERE//g' |
sed -e 's/update//g' -e 's/set//g' -e 's/where//g')

#get the table name with check
table_name=$(echo "$sql_line" | awk -F';' '{gsub(/^[ \t]+|[ \t]+$/, "",$1);print $1}')
    if ! [ -f "$table_name" ]; 
    then echo "Error : Invalid Table Name" ; return; fi

#get the column in the SET and check its existance
update_col_name=$(echo "$sql_line" | awk -F';' '{print $2}' | awk -F'=' '{gsub(/^[ \t]+|[ \t]+$/, "",$1);print $1}')

field_no=$(awk -F'|' 'BEGIN{found=0} {if (NR==1){for(i=1;i<=NF;i++)
{if($i=="'$update_col_name'")found=i}}} END{print found}' "$table_name")

#check if the enterd datatype and name is right 
col_data_type=$(awk -F'|' '{if ($1=="'$update_col_name'"){print $2}}' ".$table_name")
is_pk=$(awk -F'|' '{if ($1=="'$update_col_name'"){print $3}}' ".$table_name")
    if ((field_no == 0)); 
    then echo "Error : Invalid Column Name"; return; fi
...
selected_record=$(echo "$sql_line" | awk -F';' '{print $3}' |
awk -F'=' '{gsub(/^[ \t]+|[ \t]+$/, "",$2);print $2}')

record_field=$(awk -F'|' 'BEGIN{found=0} {if(NR!=1){for(i=1;i<=NF;i++)
{if($i=="'$selected_record'")found=i}}} END{print found}' "$table_name")

    if ((record_field == 0)); 
    then echo "Error : No Value Found"; return; fi

#validate the new value with the column type
new_value=$(echo "$sql_line" | awk -F';' '{print $2}' |
awk -F'=' '{gsub(/^[ \t]+|[ \t]+$/, "",$2);print $2}')
...
# check if the update value in pk column and the new value exists
    if ((is_pk == "pk")); 
    then
        value_exist=$(awk -v new_value="$new_value" -F'|' 'BEGIN{found=0} {if(NR!=1){for(i=1;i<=NF;i++)
        {if($"'$selected_record'"==new_value)found=1}}} END{print found}' "$table_name")
        
        if ((value_exist == 1)); 
        then echo "Error : PK Value Exists";return; fi
    fi

#update the selected record with the new value
awk -v new_value="$new_value" -i inplace -F'|' '{OFS=FS}{if($"'$record_field'"=="'$selected_record'")
{$"'$field_no'"=new_value } print}' "$table_name" 2> ../../error.log
