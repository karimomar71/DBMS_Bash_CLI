...
function delete_with_check {
	...
	sql_line=$(echo "$entry" | sed -e 's/DELETE//g' -e 's/FROM//g' -e 's/WHERE//g' |
    sed -e 's/delete//g' -e 's/from//g' -e 's/where//g')
    ...
    then echo "Error : Invalid Table Name" ; return; fi
    ...
    #if there is no where condtion then clear all table
	    if ((fields_no == 2)); 
        then
        ...
        where_column_field=$(awk -F'|' 'BEGIN{found=0} {if(NR==1){for(i=1;i<=NF;i++){if($i=="'$where_column'")found=i}}} END{print found}' "$table_name")
            if ((where_column_field == 0)); 
            then echo "Error : Invalid Where Column Name"; return; fi
        ...
        where_value_exist=$(awk -F'|' 'BEGIN{found=0} {if(NR!=1){if($"'$where_column_field'"=="'$where_value'")found=1}} END{print found}' "$table_name")
            if ((where_value_exist == 0)); 
            then echo "Warning : The Where Value does not exist in the Table "; fi
...
db_name=$1
while true; do
    tput setaf 4 #change font color to Green
    echo "+---------------------------------------------------------------------+"
