...
function select_with_check() {
    ...
    table_name=$(echo "$sql_line" | awk -F';' '{gsub(/^[ \t]+|[ \t]+$/, "",$2);print $2}') #strip fieled from all special charctars
    ...
    {if($i=="'$select_column'")found=i}}}
    END{print found}' "$table_name") #check if the coulmn name exists in the table header
    
    if [[ $select_column_field == 0 ]]  && [[ $select_column != "*" ]]; #check if invalid
    ...
    if ((fields_no == 3)); then
        ...
        if [ "$select_column" == "*" ]; then awk -v were_value="$where_value" -F'|' '{if(NR!=1){if($"'$where_column_field'" '$where_operator' were_value){print $0}}}' "$table_name" ; #checks if the column value equals * and the where colun value acheives the operation the where value
